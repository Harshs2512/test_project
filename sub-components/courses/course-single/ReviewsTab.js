...
const ReviewsTab = ({ courseID }) => {
	...
	const fetchData = async () => {
		try {
			if (courseID) {
				const res = await axios.get(`/api/reviewandrating/${courseID ? courseID : ""}`)
				// setReviews(res?.data)
			}
			else {
				console.log(courseID, "courseID is not defiend")
			}
		}
		catch (error) {
			console.log(error)
							...
							<div className="d-lg-flex align-items-center justify-content-between mb-5">
								<div className="mb-lg-0">
									<h5 className="fs-5 mb-0 text-muted">
										{item && item.username && item.username.fname} {item && item.username && item.username.lname}
									</h5>
								</div>
								<div>
