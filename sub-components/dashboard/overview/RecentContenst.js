...
const RecentCourses = ({ title }) => {
	...
	const fetchCourse = async () => {
		try {
			const { data } = await axios.get('/api/analytics/recentContest')
			setAllcourses(data)
		} catch (error) {
			console.log(error)
		}
	...
	return (
		<Card className="h-100">
			<Card.Header className="d-flex align-items-center justify-content-between card-header-height">
				...
				<Link href="/dashboard/contests/all-contests" className="btn btn-outline-secondary btn-sm">
					View all
				</Link>
			</Card.Header>
			<Card.Body>
						...
						>
							<Row>
								<Col className="col-auto">
									<Link href="#">
										<Image
											src={`/api/Contest/getthumbnail/${item._id}`}
											alt="course"
								...
								<Col className="ps-0">
									<Link href="#">
										<h5 className="text-primary-hover">{item.contest_title}</h5>
									</Link>
								</Col>
							</Row>
						</ListGroup.Item>
