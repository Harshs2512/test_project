...
const ChallengesCard = (props) => {
  ...
  .filter(item => parseInt(item.currentprice) > 0 && item.is_published === 'live');
  const handleShow = async (currentId) => {
    try {
        const ContestData = await axios.get(`/api/Contest/${currentId}`);
        const SingleData = ContestData && ContestData.data && ContestData.data.contest
        const serializedProblem = JSON.stringify(SingleData);
        const encodedProblem = encodeURIComponent(serializedProblem);
        const url = `/studio/contest-single/single?problem=${encodedProblem}`;
        router.push(url);
    } catch (error) {
