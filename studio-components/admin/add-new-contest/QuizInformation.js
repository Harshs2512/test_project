...
const QuizInformation = () => {
  ...
  const { contestId, action } = router.query;
  ...
  const fetchContest = async () => {
    try {
      const response = await axios.get(`/api/Contest/${contestId}`);
    ...
    try {
      ...
      formDatas.append(
        ...
        JSON.stringify(
          questionsList.map((question) => ({
            ...
            Function_data: question.Function_data,
            problem_Statement: question.Problem_statement,
            problem_Constraints: question.constrants,
            problem_Examplation: question.problem_Examplation,
            example: question.example,
      ...
      if (action === "update") {
        const response = await axios.put(
          `/api/Contest/crudContest?contestId=${contestId}`,
          formDatas
        );
    ...
    <Form onSubmit={handleFormSubmit}>
      ...
      <Card className="mb-3">
        <Card.Header className="border-bottom px-4 py-3">
          <h3 className="mb-0">
            <u>Basic Information for Create Contest</u>
          </h3>
        </Card.Header>
        <Card.Body>
                ...
                <Row>
                  <Col>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Select
                        value={questionsList[questionIndex]?.problem_level}
                        onChange={(e) =>
                          setQuestionsList((prevList) =>
                            prevList.map((q, i) =>
                              i === questionIndex
