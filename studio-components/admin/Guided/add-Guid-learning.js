...
import { GuidEditor, ReactQuillEditor, ProblemExplanaton } from "widgets";
      ...
      prevTopics.map((topic, tIndex) =>
        tIndex === topicIndex
          ? {
              ...
              questionsList: topic.questionsList.map((question, qIndex) =>
                qIndex === questionIndex
                  ? { ...question, problem_Examplation: value }
                  : question
              ),
            }
          : topic
    ...
    try {
      ...
      if (action === "update") {
        const response = await axios.put(
          `/api/Course-guide/guided?guideId=${guideId}`,
          formDatas
        );
