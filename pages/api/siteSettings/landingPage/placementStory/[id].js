...
const getHandler = async (req, res) => {
    await connectDB();
    try {
        const records = await placementStoryModel.findById(req.query.id).select('-student_icon').select('-company_logo');
        if (records) {
...
const deleteHandler = async (req, res) => {
    await connectDB();
    try {
        const records = await placementStoryModel.findByIdAndDelete(req.query.id);
        if (records) {
            res.status(200).json(records)
        } else {
            res.status(404).send({ message: 'No categories found' });
