...
const getHandler = async (req, res) => {
    await connectDB();
    try {
        const records = await circularCarouselModel.findById(req.query.id).select('-company_logo');
        if (records) {
...
const deleteHandler = async (req, res) => {
    await connectDB();
    try {
        const records = await circularCarouselModel.findByIdAndDelete(req.query.id);
        if (records) {
            res.status(200).json(records)
        } else {
            res.status(404).send({ message: 'No categories found' });
