...
const PrintInvoiceDetails = React.forwardRef(({ orderId, onClose},ref) => {
	...
	useEffect(() => {
		const fetchUserDetails = async (userId) => {
			try {
				...
				console.error('Error fetching user details download Pdf 1', error);
				return null;
			}
		};
	...
	const userDetail = useCallback(async () => {
		try {
			...
			console.error("Error fetching user data download pdf:", error);
		}
	}, [userId])
	useEffect(() => {
		userDetail();
	...
	return (
		<Card className="border-0 " display='none' ref={ref}>
			<Button onClick={onClose}>Close</Button>
			<Card.Body id="invoice-container">
				{/* Card body */}
														...
														<Col className="ps-0">
															<Link href="#">
																<h5 className="text-primary-hover">
																	{data.course_title || data.assignmentName || data.title}
																</h5>
															</Link>
															<div className="d-flex align-items-center">
																...
																/>
																<span className="fs-6">
																	{userDetails[data && data.created_by] ? userDetails[data && data.created_by].fname + ' ' + userDetails[data && data.created_by].lname : 'User Name'}
																</span>
															</div>
														</Col>
														<Col className="col-auto">
