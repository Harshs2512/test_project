...
const BuyPage = () => {
	...
	const [BuyQuiz, setBuyQuiz] = useState('');
	const session = useSession()
	let price = 0;
	let discount = 0;
	let paidAmount = 0;
	const currentprice = Number(BuyQuiz && BuyQuiz.currentprice);
	const actualprice = Number(BuyQuiz && BuyQuiz.actualprice);
	price += currentprice;
	...
	const data = async () => {
		...
		setBuyQuiz(res && res.data && res.data.quiz);
	};
	useEffect(() => {
		if (Id) {
			data();
	...
	const orderData = {
		user_detail: userID,
		purchase_item: BuyQuiz,
		total_item: BuyQuiz.length,
										...
										>
											<Row>
												<Col className="col-auto">
													<Link href="#">
														<Image
															src={`/api/quiz/quizthumbnail/${BuyQuiz._id}`}
												...
												<Col className="ps-0">
													...
													<h5 className="mb-2">Mock Level :  {BuyQuiz.level ? BuyQuiz.level : "Intermediate"}</h5>
													<div className="d-flex align-items-center">
														...
														/>
														<span className="fs-6">{data.instructor_name}</span>
													</div>
												</Col>
												<Col className="col-auto">
													₹{BuyQuiz.currentprice}
												</Col>
											</Row>
										</ListGroup.Item>
							...
							<Card className="border-0 mb-3 mb-lg-0 sticky-top">
								<div className="p-5 text-center">
									...
									<div className="d-flex justify-content-center">
										...
										<div className="display-4 fw-bold text-primary">₹{paidAmount}</div>
									</div>
								</div>
								<hr className="m-0" />
								<div className="p-5">
										...
										<ListGroup.Item as="li" className="mb-2" bsPrefix=" ">
											<div className="d-flex align-items-center justify-content-between ">
												<div className="mb-1 mb-lg-0 ">
													<span>{BuyQuiz && BuyQuiz.title}</span>
												</div>
												<div>
													<h5>&#x20B9;{BuyQuiz && BuyQuiz.currentprice}</h5>
												</div>
											</div>
										</ListGroup.Item>
									</ListGroup>
