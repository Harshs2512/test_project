...
const Codedesign = () => {
  ...
  const currentQuestion = problem ? problem.questionsList[0] : "blank value";
  const currentTestCase = currentQuestion?.testCases;
  const functionData = problem?.questionsList?.map((data) => data.Function_data) || [];
  ...
  const fetchContest = async () => {
    try {
      ...
      setContest(res && res.data && res.data.Contests);
    } catch (error) {
      console.log(error);
    }
  };
    ...
    <>
      ...
      <Row className={`relative flex flex-col flex-1 mx-6 bg-white`}>
        <Col
          ...
          md={12}
          className="mt-1"
          style={{height: isFullScreen ? "850px" :"750px", overflow:"scroll" }}
        >
          ...
          >
            <Tab eventKey="question" title="Problem">
              <Question
                ...
                Title={problem ? problem.contest_title : "default title"}
              />
            </Tab>
