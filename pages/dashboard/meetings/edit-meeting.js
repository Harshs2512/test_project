...
const Editmeeting = () => {
    ...
    useEffect(() => {
        const fetchMeetings = async () => {
            try {
                const res = await axios.get(`/api/zoom/${pathId}`);
                if (res.status === 200) {
                    const fetchedData = res.data;
                    setFormData(prevFormData => ({
                        ...prevFormData,
                        topic: fetchedData.topic || 'My Meeting',
                        agenda: fetchedData.agenda || '',
                        date: fetchedData.start_time.split('T')[0],
                        time: fetchedData.start_time.split('T')[1].slice(0, 5),
                        durationTime: Math.floor(fetchedData.duration / 60),
                        durationMinute: fetchedData.duration % 60,
                        // pmi: fetchedData.id,
    ...
    const submitData = async () => {
        ...
        try {
            const res = await axios.patch(`/api/zoom/${pathId}`, data);
            if (res.status === 200) {
                ...
                <Col lg={12} md={12} sm={12}>
                    <div className="border-bottom pb-4 mb-4 d-md-flex align-items-center justify-content-between">
                        <div className="mb-3 mb-md-0">
                            <h1 className="mb-1 h2 fw-bold">Edit Meetings</h1>
                            <Breadcrumb>
                ...
                <Col lg={12} md={12} sm={12}>
                    <Card>
                        <Card.Header className="d-flex justify-content-between align-items-center border-bottom card-header-height mb-4" >
                            <h4 className="mb-0">Edit Meeting</h4>
                        </Card.Header>
