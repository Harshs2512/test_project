...
const AllCourseLogo = () => {
    ...
    const getPosts = async () => {
        try {
            const res = await axios.get("/api/siteSettings/landingPage/courseLogo/getRecords");
            if (res.status === 200) {
            ...
            },
            {
                ...
                Cell: ({ value, row }) => {
                    return (
                        <div className='d-flex'>
                            <Image
                                src={`/api/siteSettings/landingPage/courseLogo/Courselogo/${row.original._id}`}
                                alt=""
    ...
    const actionHandler = async (id, action) => {
        ...
        if (action === 'update') {
            setModalOpen(true)
            try {
                await axios.get(`/api/siteSettings/landingPage/courseLogo/${id}`)
                    .then((response) => {
        ...
        if (action === 'delete') {
            try {
                const res = await axios.delete(`/api/siteSettings/landingPage/courseLogo/${id}`);
                getPosts()
    ...
    const submitData = async () => {
        if (actiontype === 'update') {
            try {
                ...
                data.append("course_logo", courseImage);
                const res = await axios.put("/api/siteSettings/landingPage/courseLogo/getRecords", data);
                if (res.status === 201) {
            ...
            try {
                ...
                data.append("course_name", course_name);
                data.append("course_logo", courseImage);
                const res = await axios.post("/api/siteSettings/landingPage/courseLogo/getRecords", data);
                if (res) {
                    toast.success("Created course logo Success fully");
                    getPosts();
                } else {
                    toast.info("course Logo Already Exist");
                };
                setModalOpen(false);
            }
            catch (err) {
        ...
        <Fragment>
                ...
                <Col lg={12} md={12} sm={12}>
                    <div className="border-bottom pb-4 mb-4 d-md-flex align-items-center justify-content-between">
                        <div className="mb-3 mb-md-0">
                            <h1 className="mb-1 h2 fw-bold">Course Logo</h1>
                            <Breadcrumb>
                                <Breadcrumb.Item href="#">Dashboard</Breadcrumb.Item>
                                <Breadcrumb.Item href="#">Landing</Breadcrumb.Item>
                                <Breadcrumb.Item active>Course Logo</Breadcrumb.Item>
                            </Breadcrumb>
                        </div>
                        <div>
            ...
            <Modal show={modalOpen} onHide={handleCloseModal}>
                <Modal.Header closeButton>
                    <Modal.Title>Add Course Logo</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form>
