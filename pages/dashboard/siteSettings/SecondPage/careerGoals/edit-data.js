...
const EditData = () => {
    ...
    const fetchData = async () => {
        try {
            if (id) {
                const { data } = await axios.get(`/api/siteSettings/secondPage/careerGoals/${id}`);
                setFormData({
    ...
    const submitData = async () => {
        ...
        try {
            const res = await axios.put(`/api/siteSettings/secondPage/careerGoals/updateRecord`, data);
            if (res.status === 200) {
