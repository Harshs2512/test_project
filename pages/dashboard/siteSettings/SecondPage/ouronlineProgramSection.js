...
const OurPrograms = () => {
    ...
    const getPosts = async () => {
        try {
            const res = await axios.get("/api/siteSettings/secondPage/ouronlineprograms/getRecord");
            if (res.status === 200) {
                ...
                Cell: ({ value, row }) => {
                    ...
                    return (
                        <div className="mb-0 d-flex w-10">
                            <Image src={`/api/siteSettings/secondPage/ouronlineprograms/getlogo/${row.original._id}`} className='img-fluid' alt={value?.catName} />
    ...
    const actionHandler = async (id, action) => {
        ...
        if (action === 'update') {
            setModalOpen(true)
            try {
                await axios.get(`/api/siteSettings/secondPage/ouronlineprograms/`)
                    .then((response) => {
                        setFormData({
        ...
        if (action === 'delete') {
            try {
                const res = await axios.delete(`/api/siteSettings/secondPage/ouronlineprograms/${id}`);
                getPosts()
    ...
    const submitData = async () => {
        if (actiontype === 'update') {
            try {
                ...
                const res = await axios.put(`/api/siteSettings/secondPage/ouronlineprograms/updateRecord`, data);
                if (res.status === 201) {
            ...
            try {
                ...
                const res = await axios.post("/api/siteSettings/secondPage/ouronlineprograms/addRecord", data);
                if (res.status === 201) {
                    toast.success("Created Success fully");
                    setFormData({
                        category: "",
    ...
    const updateStatus = async (id, action) => {
        try {
            const res = await axios.put(`/api/siteSettings/secondPage/ouronlineprograms/${id}`, { action })
            if (res.status === 201) {
                ...
                <Col lg={12} md={12} sm={12}>
                    <div className="border-bottom pb-4 mb-4 d-md-flex align-items-center justify-content-between">
                        <div className="mb-3 mb-md-0">
                            <h1 className="mb-1 h2 fw-bold">Online Programms</h1>
                            <Breadcrumb>
                                <Breadcrumb.Item href="#">Dashboard</Breadcrumb.Item>
                                <Breadcrumb.Item href="#">Landing</Breadcrumb.Item>
                                <Breadcrumb.Item active>Online programms</Breadcrumb.Item>
                            </Breadcrumb>
                        </div>
                        <div>
                ...
                <Col lg={12} md={12} sm={12}>
                    <Card>
                        <Card.Header className="d-flex justify-content-between align-items-center border-bottom-0 card-header-height">
                            <h4 className="mb-0">Programms</h4>
                        </Card.Header>
                    ...
                    <Form>
                        <Form.Group id="question" className='mb-3'>
                            ...
                            >
                                ...
                                {category
                                    .filter((item) => !allposts.some((post) => post.category === item._id))
                                    .map((value, index) => (
