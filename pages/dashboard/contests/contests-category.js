...
const CoursesCategory = () => {
  ...
  const getContest = async () => {
    try {
      const { data } = await axios.get("/api/Contest/crudCategory");
      setCategories(data);
    } catch (error) {
  ...
  const hadleDelete = async (id) => {
    try {
      const { data } = await axios.delete(`/api/Contest/category/${id}/`);
      if (data) {
  ...
  const getcategoryUpdate = async (id) => {
    try {
      ...
      const res = await axios.get(`/api/Contest/category/${id}`)
  ...
  const handleUpdate = async ({ catName, _id }) => {
    try {
      const result = await axios.put(`/api/Contest/category/${_id}`, {
        catName,
      });
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        accessor: "updatedAt",
        Header: "Date Updated",
        Cell: ({ value }) => formatDate(value), // Apply date formatting to updatedAt column
      },
    ],
  ...
  const submitHandler = async ({ catName }) => {
    try {
      const result = await axios.post("/api/Contest/crudCategory", {
        catName,
      });
      toast.success("Contests Category is Created Successfully");
      setModalopen(false);
      getContest();
    } catch (error) {
      toast.error("This category is exist!");
        ...
        <Col lg={12} md={12} sm={12}>
          <div className="border-bottom pb-4 mb-4 d-md-flex align-items-center justify-content-between">
            <div className="mb-3 mb-md-0">
              <h1 className="mb-1 h2 fw-bold">Contest Category</h1>
              <Breadcrumb>
                <Breadcrumb.Item href="#">Dashboard</Breadcrumb.Item>
                <Breadcrumb.Item href="#">Contest</Breadcrumb.Item>
                <Breadcrumb.Item active>Contest Category</Breadcrumb.Item>
              </Breadcrumb>
            </div>
            <div>
              <Can I="createContestCategory">
