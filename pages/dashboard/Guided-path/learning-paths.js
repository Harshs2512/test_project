...
const AllGuides = () => {
  ...
  const handlePrice = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put(`/api/Course-guide/updateprice`, {
        currentprice,
        actualprice,
        courseId,
      });
      if (res) {
        toast.success(`Learning path Price set successfully`);
        AllLearning();
      } else {
  ...
  const AllLearning = async () => {
    try {
      const Contest = await axios.get("/api/Course-guide/guided");
      const Contestdata = Contest?.data?.Learning;
  ...
  const CoursesTable = ({ courses_data }) => {
            ...
            return (
              <Link href="#" className="text-inherit">
                <div className="d-lg-flex align-items-center">
                  <div>
                    <Image
                      src={`/api/Course-guide/getthumbnail/${row.original._id}`}
                      alt=""
        ...
        swalWithBootstrapButtons
          ...
          .then((result) => {
            if (result.isConfirmed) {
              const res = axios.delete(`/api/Course-guide/${id}`);
              if (res) {
                AllLearning();
              } else {
                toast.error("something went wrong");
    ...
    const handleUpdate = async (contestId,action) => {
      router.push(`/studio/admin/add-guid-path?guideId=${contestId}&action=${action}`);
    };
    const handleUpdateStatus = async (id, status) => {
      try {
        const res = await axios.put(`/api/Course-guide/updatestatus`, {
          id: id,
          is_published: status,
        });
        if (res.data.updatedContest.is_published === "live") {
          ...
          toast.warning(`Learning path is now pending`);
          AllLearning();
        } else if (res.data.updatedContest.is_published === "reject") {
          toast.warning(`Learning path is now reject`);
          AllLearning();
        } else {
          toast.error("something went wrong");
        }
        ...
        <Col lg={12} md={12} sm={12}>
          <div className="border-bottom pb-4 mb-4 d-md-flex align-items-center justify-content-between">
            <div className="mb-3 mb-md-0">
              <h1 className="mb-1 h2 fw-bold">All Guided Path</h1>
              <Breadcrumb>
            ...
            <div>
              <Can I="createContest">
              <Link href="/studio/admin/add-guid-path" className="btn btn-info">
                  Add Learning Guide
                </Link>
              </Can>
            </div>
