...
const AllCourses = () => {
	...
	const [allapprovedcourses, setAllapprovedcourses] = useState([])
	const [allpendingcourses, setAllpendingcourses] = useState([])
	const [allrejectedcourses, setAllrejectedcourses] = useState([])
	...
	const getallcourse = async () => {
		setLoading(true);
		try {
			const course = await axios.get('/api/courses/getallcourse');
			setAllData(course.data.courses);
			const courses = course.data.courses.map((post) => {
				return {
					...post,
					is_published: post.is_published === 'live' ? 'live' : post.is_published === 'reject' ? 'reject' : 'pending',
				};
			});
			const publishedPosts = courses.filter((post) => post.is_published === 'live');
			const disabledPosts = courses.filter((post) => post.is_published === 'pending');
			const rejectedPosts = courses.filter((post) => post.is_published === 'reject');
			setAllapprovedcourses(publishedPosts);
			setAllpendingcourses(disabledPosts);
			setAllrejectedcourses(rejectedPosts);
		}
		catch (error) {
			console.log(error)
		}
	...
	const handlePrice = async (e) => {
		e.preventDefault();
		try {
			const res = await axios.put(`/api/courses/updateprice`, { currentprice, actualprice, courseId });
			if (res) {
	...
	const CoursesTable = ({ courses_data }) => {
							...
							(<Link href="#" className="text-inherit">
								<div className="d-lg-flex align-items-center">
									...
									<div className="ms-lg-3 mt-2 mt-lg-0">
										<Link href={`/marketing/courses/course-resume/${row.original.slug}`}>
						...
						if (row.original.is_published === 'pending') {
							return (
								<Fragment>
									...
									<Button href="#" variant="success" className="btn-sm" onClick={() => handleUpdate(row.original, 'live')} >
										Approved
									</Button>
								</Fragment>
							);
		...
		const handleUpdate = async (id, status) => {
			try {
				const isAllApproved = id.section.every((section) => {
					const lecturesApproved = section.lecture.every((lecture) => {
						return lecture.isApproved === 'true';
					});
					return lecturesApproved;
				});
				if (isAllApproved) {
					...
					toast.warning("All lectues are not approved")
				}
			} catch (error) {
				toast.error("Somtihing went wrong");
				console.log(error)
			...
			try {
				...
				swalWithBootstrapButtons.fire({
					...
					if (result.isConfirmed) {
						const res = axios.delete(`/api/courses/${id}`)
						if (res) {
							getallcourse();
						}
						else {
												...
												<Dropdown.Menu align="end">
													...
													<Can I="deleteCourse">
														<Dropdown.Item eventKey="1" onClick={() => handleDelete(rowData._id)}>
															<XCircle size="15px" className="me-1" />Reject and Delete Course
														</Dropdown.Item>
													</Can>
													<Can I="editCourse">
														...
														<Dropdown.Item eventKey="1" onClick={() => handlePriceUpdate(rowData._id, rowData.actualprice, rowData.currentprice)}>
															<XCircle size="15px" className="me-1" />Edit
														</Dropdown.Item>
													</Can>
												</Dropdown.Menu>
											</Dropdown>
			...
			<Row>
				<Col lg={12} md={12} sm={12}>
					<div className="border-bottom pb-4 mb-4 d-md-flex align-items-center justify-content-between">
						...
						<div>
							<Can I="createCourse">
								<Link href="/dashboard/courses/offline-courses/add-new-course" className="btn btn-primary">
									Add New Courses
								</Link>
							</Can>
						</div>
					...
					<Form>
						<Col className="mb-3">
							<Form.Group className="mb-3" controlId="formcurrentprice">
								...
								/>
								{currentpriceError && <div className="text-danger">{currentpriceError}</div>}
							</Form.Group>
						</Col>
						<Col className="mb-3">
							<Form.Group className="mb-3" controlId="formDiscountedPrice">
								...
								/>
								{discountedPriceError && <div className="text-danger">{discountedPriceError}</div>}
							</Form.Group>
						</Col>
						<Button variant="primary" className="m-1" type="submit" onClick={handlePrice}>
							Update
						</Button>
					</Form>
				</Modal.Body>
